...
def create_train_static_shape_cell(network, opt, config):
...
def create_train_static_shape_fn(network, optimizer, config):
    from mindspore.amp import all_finite, StaticLossScaler
    network.set_train()

    loss_scaler = StaticLossScaler(config.loss_scale // 2)
    ...
    grad_fn = ops.value_and_grad(forward_func, grad_position=None, weights=optimizer.parameters, has_aux=False)
    ...
    def train_step(*inputs):
        ...
        if not grads_finite:
            print("this step overflow, still update.")

        return loss_scaler.unscale(loss)

    return train_step

def create_train_static_shape_fn_gradoperation_with_sens(network, optimizer, config):
    ...
    grad_fn = ops.GradOperation(get_by_list=True, sens_param=True)(network, optimizer.parameters)
    sens_value = config.loss_scale // 2

    @ms.ms_function
    def train_step(*inputs):
        loss = network(*inputs)
        sens = ops.fill(loss.dtype, loss.shape, sens_value)
...
def create_train_static_shape_fn_gradoperation_with_lossscale(network, optimizer, config):
    from mindspore.amp import all_finite, StaticLossScaler
    network.set_train()
    ...
    grad_fn = ops.GradOperation(get_by_list=True, sens_param=False)(forward_func, optimizer.parameters)

    @ms.ms_function
    def train_step(*inputs):
        loss = forward_func(*inputs)
...
def run_train():
    ...
    if config.ms_strategy == "StaticCell":
        train_step = create_train_static_shape_cell(network, opt, config)
    elif config.ms_strategy == "StaticShape":
        if config.ms_use_gard_operation:
            if config.ms_use_sens:
                train_step = create_train_static_shape_fn_gradoperation_with_sens(network, opt, config)
            else:
                train_step = create_train_static_shape_fn_gradoperation_with_lossscale(network, opt, config)
        else:
            train_step = create_train_static_shape_fn(network, opt, config)
    else:
    ...
    for epoch_idx in range(config.max_epoch):
        for step_idx, data in enumerate(data_loader):
            ...
            loss = train_step(images, data[2], data[3], data[4], data[5], data[6],
                              data[7], input_shape)
